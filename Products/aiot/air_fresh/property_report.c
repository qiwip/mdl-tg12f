...
void user_post_property(property_report_msg_t * msg)
{
    ...
    cJSON_AddNumberToObject(response_root, "powerstate", msg->powerstate);
    cJSON_AddNumberToObject(response_root, "windspeed", msg->speed);
    cJSON_AddNumberToObject(response_root, "ECOOnOff", msg->bypass);
...
void report_device_property(char *seq, int flag)
{
    ...
    msg.powerstate = device_status->powerstate;
    msg.speed = device_status->speed;
    msg.bypass = device_status->bypass;
...
void process_property_report_task(void)
{
    unsigned int rcvLen;
    property_report_msg_t msg;
...
#ifndef REPORT_MULTHREAD
void process_detect_property_change(void)
{
    aos_msleep(1000);
    user_example_ctx_t *user_example_ctx = user_example_get_ctx();
    device_status_t *device_status = &user_example_ctx->status;
...
{
    LOG("start detect property change");
    while (1) {
        aos_msleep(1000);
        user_example_ctx_t *user_example_ctx = user_example_get_ctx();
        device_status_t *device_status = &user_example_ctx->status;
